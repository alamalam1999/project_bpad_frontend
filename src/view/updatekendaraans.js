...
function UpdatedKendaraans() {
    ...
    useEffect(() => {
        const fetchBook = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/user/datakendaraan/${id}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/user/updatekendaraan/${id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
            ...
            if (response.ok) {
                ...
                navigate('/kendaraan'); // Redirect after successful update
            } else {
    ...
    return (
        ...
        atribut: '', */}
                    <div className="mb-3">
                        ...
                        <input type="text" name='nopol' value={formData.nopol} onChange={handleChange} className="form-control" />
                    </div>
                    <div className="mb-3">
                        <label className="form-label">Nip</label>
                        <input type="text" name='jenis_roda' value={formData.jenis_roda} onChange={handleChange} className="form-control" />
                    </div>
                    <div className="mb-3">
                        <label className="form-label">Atribut</label>
                        <input type="text" name='merk' value={formData.merk} onChange={handleChange} className="form-control" />
                    </div>


                    <div className="mb-3">
                        <label className="form-label">Nama Asn</label>
                        <input type="text" name='jenis_kendaraan' value={formData.jenis_kendaraan} onChange={handleChange} className="form-control" />
                    </div>
                    <div className="mb-3">
                        <label className="form-label">Nip</label>
                        <input type="text" name='tahun_kendaraan' value={formData.tahun_kendaraan} onChange={handleChange} className="form-control" />
                    </div>
                    <div className="mb-3">
                        <label className="form-label">Atribut</label>
...
export default UpdatedKendaraans;
