...
function UpdatedInstansis() {
    ...
    useEffect(() => {
        const fetchBook = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/user/datainstansi/${id}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/user/updateinstansi/${id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
            ...
            if (response.ok) {
                ...
                navigate('/instansi'); // Redirect after successful update
            } else {
            ...
            <div className='mt-3'>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        ...
                        <input type="text" name='nama_instansi' value={formData.nama_instansi} onChange={handleChange} className="form-control" />
                    </div>
...
export default UpdatedInstansis;
